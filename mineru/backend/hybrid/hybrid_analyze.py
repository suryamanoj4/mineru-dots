...
from mineru.backend.vlm.dots_ocr_client import DotsOCRClient
...
def get_batch_ratio(device):
    ...
    if env_val:
        try:
            ...
            logger.warning(
                f"Invalid MINERU_HYBRID_BATCH_RATIO value: {env_val}, switching to auto mode. Error: {e}"
            )
...
def _should_enable_vlm_ocr(
    ...
    """判断是否启用VLM OCR"""
    force_enable = os.getenv("MINERU_FORCE_VLM_OCR_ENABLE", "0").lower() in (
        "1",
        "true",
        "yes",
    )
    ...
    force_pipeline = os.getenv("MINERU_HYBRID_FORCE_PIPELINE_ENABLE", "0").lower() in (
        "1",
        "true",
        "yes",
    )
