...
@click.option(
    ...
    type=str,
    default=None,
    help="Route parsing requests to a MinerU FastAPI backend instead of running parsing in-process.",
)
@click.option(
    '--backend-options',
    'backend_options',
...
def main(ctx,
    ...
    i18n = gr.I18n(
        ...
        zh={
            ...
            "backend_info_lite": "轻量化 Tesseract 直接后端，不使用完整的 MinerU 流水线。",
...
def get_backend_info(backend_choice):
    ...
    configured_backend_api_url = backend_api_url or os.getenv("MINERU_GRADIO_BACKEND_API_URL")
    if configured_backend_api_url:
        os.environ["MINERU_GRADIO_BACKEND_API_URL"] = configured_backend_api_url
