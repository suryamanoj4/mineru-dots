...
If your question is not listed, try using [DeepWiki](https://deepwiki.com/opendatalab/VParse)'s AI assistant for common issues.

For unresolved problems, join our [Discord](https://discord.gg/Tdedn9GTXq) or [WeChat](https://vparse.net/community-portal/?aliasId=3c430f94) community for support.
...
??? question "Encountered the error `ImportError: libGL.so.1: cannot open shared object file: No such file or directory` in Ubuntu 22.04 on WSL2"
    ...
    Reference: [#388](https://github.com/opendatalab/VParse/issues/388)


??? question "Missing text information in parsing results when installing and using on Linux systems."

    VParse uses `pypdfium2` instead of `pymupdf` as the PDF page rendering engine in versions >=2.0 to resolve AGPLv3 license issues. On some Linux distributions, due to missing CJK fonts, some text may be lost during the process of rendering PDFs to images.
    ...
    Reference: [#2915](https://github.com/opendatalab/VParse/issues/2915)
