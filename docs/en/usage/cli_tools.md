...
To view help information for VParse command line tools, you can use the `--help` parameter. Here are help information examples for various command line tools:
```bash
vparse --help
Usage: vparse [OPTIONS]
...
Some parameters of VParse command line tools have equivalent environment variable configurations. Generally, environment variable configurations have higher priority than command line parameters and take effect across all command line tools.
...
- `VPARSE_HYBRID_BATCH_RATIO`:
    ...
    * Single Client VRAM Size | VPARSE_HYBRID_BATCH_RATIO
      ------------------------|--------------------------
      <= 6   GB               | 8
      <= 4.5 GB               | 4
      <= 3   GB               | 2
...
- `VPARSE_HYBRID_FORCE_PIPELINE_ENABLE`:
...
- `VPARSE_VL_MODEL_NAME`:
    * Used to specify the model name for the vlm/hybrid backend, allowing you to designate the model required for VParse to run when multiple models exist on a remote openai-server.
