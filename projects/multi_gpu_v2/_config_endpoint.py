...
def config_endpoint():
    ...
    If the Hugging Face endpoint is reachable, it sets VPARSE_MODEL_SOURCE to 'huggingface'.
    Otherwise, it falls back to 'modelscope'.
    """

    os.environ.setdefault('VPARSE_MODEL_SOURCE', 'huggingface')
    ...
    if os.environ['VPARSE_MODEL_SOURCE'] == 'huggingface':
        try:
    ...
    elif os.environ['VPARSE_MODEL_SOURCE'] == 'modelscope':
        try:
    ...
    elif os.environ['VPARSE_MODEL_SOURCE'] == 'local':
        logging.info("Using 'local' as model source.")
        return True
    
    else:
        logging.error(f"Using custom model source: {os.environ['VPARSE_MODEL_SOURCE']}")
        return True
