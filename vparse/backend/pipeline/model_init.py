...
from vparse.model.ocr.pytorch_paddle import PytorchPaddleOCR
...
if MFR_MODEL.lower() in ['true', '1', 'yes']:
    ...
    logger.warning(f"Invalid VPARSE_FORMULA_CH_SUPPORT value: {MFR_MODEL}, set to default 'False'")
    MFR_MODEL = "unimernet_small"
...
class HybridModelSingleton:
    ...
    def get_model(
        ...
        if key not in self._models:
            self._models[key] = VParseHybridModel(
                lang=lang,
...
MineruPipelineModel = VParsePipelineModel
MineruHybridModel = VParseHybridModel
