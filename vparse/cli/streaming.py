...
from vparse.cli.common import convert_pdf_bytes_to_bytes_by_pypdfium2, do_parse
...
def iter_stream_parse(
    ...
    **kwargs,
    ...
    preview_content_list_v2_path = session_root / f"{pdf_file_name}_preview_content_list_v2.json"
    preview_pdf_path = session_root / f"{pdf_file_name}_preview.pdf"

    backend_label = "pipeline" if backend in {"pipeline", "lite"} else "vlm"
    ...
    try:
        for page_index in range(total_pages):
            ...
            if backend in {"pipeline", "lite"} and isinstance(page_model, list):
            ...
            elif isinstance(page_model, list):
                preview_model.extend(page_model)
            elif page_model is not None:
                preview_model.append(page_model)
...
def stream_parse(
    ...
    f_dump_md: bool = True,
    f_dump_middle_json: bool = False,
    f_dump_model_output: bool = False,
    f_dump_orig_pdf: bool = False,
    f_dump_content_list: bool = True,
    f_dump_content_list_v2: bool = False,
    **kwargs,
) -> str:
    session_root = None
    write_manifest = None
