...
from vparse.cli.common import aio_do_parse, do_parse, read_fn, pdf_suffixes, image_suffixes
from vparse.cli.streaming import iter_stream_parse, cleanup_stream_session
...
def _create_stream_zip(job: dict) -> str:
    ...
    zip_fd, zip_path = tempfile.mkstemp(suffix=".zip", prefix="vparse_stream_results_")
    os.close(zip_fd)
...
def _get_parse_dir(output_dir: str, pdf_name: str, backend: str, parse_method: str) -> str:
    if backend == "lite":
        return os.path.join(output_dir, pdf_name, f"lite_{parse_method}")
            ...
            with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
                for pdf_name in pdf_file_names:
                    ...
                    if backend == "pipeline":
                        ...
                        parse_dir = os.path.join(unique_dir, pdf_name, f"lite_{parse_method}")
...
def main(ctx, host, port, reload, **kwargs):
    ...
    uvicorn.run("vparse.cli.fast_api:app", host=host, port=port, reload=reload)
